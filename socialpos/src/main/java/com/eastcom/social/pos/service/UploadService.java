...
public class UploadService extends Service {
	...
	public class GetDataTask extends AsyncTask<Void, Void, Boolean> {
		...
		protected Boolean doInBackground(Void... params) {
			try {
				...
				getDataTask = null;
				MyLog.w("uploadService", "" + e.getMessage());
				return false;
			}
			return true;
		}
	...
	private void uploadTradeService() {
		List<Trade> list = tradeService.queryByIsUpload();
		for (int i = 0; i < list.size(); i++) {

			Trade trade = list.get(i);
			if (trade.getTrade_time().after(TimeUtil.getDate("2017-10-22"))) {
	...
	private void uploadTradeDetailService() {
		...
		for (int i = 0; i < list.size(); i++) {
			List<Trade> list2 = tradeService.queryById(list.get(i)
					.getFk_trade_id());
			if (list2.size() > 0) {
				Trade trade = list2.get(0);
				// 交易汇总上传失败，不上传明细
				if (trade.getIs_upload() == 0) {
					if (trade.getTrade_time().after(TimeUtil.getDate("2017-10-22"))) {
	...
	private void sendTradeMessage(Trade mTrade) {
		...
		if (mTrade.getEncrypt() == null) {// 只有社保卡消费需要验证是否加密上传
			...
			MyLog.i("slow socialCardNo", socialCardNo + "----" + bankCard
					+ mTrade.getTrade_type());
			if (mTrade.getTrade_type() == Integer
					.valueOf(SoTradeType.金融交易_82字节)) {
				...
				FinanceTradeMessage financeTradeMessage = new FinanceTradeMessage(
						...
						psamNo, terminalCode, rfsamNo, tradeRandom, mac,
						sendCount, tradeNo);
