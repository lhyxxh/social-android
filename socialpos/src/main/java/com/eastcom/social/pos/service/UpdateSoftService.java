	...
	public class GetDataTask extends AsyncTask<Void, Void, Boolean> {
		...
		protected Boolean doInBackground(Void... params) {
			try {
				...
				MyLog.i("UpdateSoftService", "UpdateSoftMessage Exception");
				getDataTask = null;
				e.printStackTrace();
				return false;
			}
		...
		private void mergeFile(String fileName, int versionSize, String path) {
			...
			for (int i = 0; i < versionSize; i++) {
				File tempFile = new File(Constance.ApkDownloadFile + path, ""
						+ i);
				tempFiles.add(tempFile);
			}
			String realFilePath = Constance.ApkDownloadFile + fileName;
